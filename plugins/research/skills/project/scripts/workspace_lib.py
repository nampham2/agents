...
# Reported at every promotion, so the pressure is visible at the moment a person can act on it
# instead of at the next validation. Same lever Claude Code's own auto memory uses on its index.
MEMORY_INDEX_WARN_RATIO = 0.85
...
def render_memory_index(workspace_root: Path) -> str:
    ...
    parts = [
        ...
        "\n",
        "Pointers only. Open a topic with `research-project read-memory <name>` when its description\n",
        "matches the work in hand, and rank topics for a query with `research-project search-memory`.\n",
    ]
    for kind in MEMORY_KINDS:
        # Scope stays in the frontmatter, where ranked search reads it. Measured on a real root it was
        # 32% of every pointer's bytes, and the budget binds on bytes.
        chosen = sorted(
            (topic for topic in topics if topic.kind == kind and topic.status == "active"), key=lambda topic: topic.name
        )
        if not chosen:
            continue
        ...
        for topic in chosen:
            parts.append(
                f"- [{topic.name}]({MEMORY_DIRECTORY}/{topic.name}.md) — {_escape_table(topic.description)}\n"
            )
...
def index_headroom(workspace_root: Path) -> "dict[str, Any]":
    """How much of the discovery budget `MEMORY.md` uses, with a warning from 85% of either bound."""
    path = memory_index_path(workspace_root)
    content = ""
    if path.is_file():
        try:
            content = read_text(path)
    ...
    warnings: list[str] = []
    if size >= MEMORY_INDEX_MAX_BYTES * MEMORY_INDEX_WARN_RATIO:
        warnings.append(
            f"{path} is at {size} of {MEMORY_INDEX_MAX_BYTES} bytes; merge or retire topics before the budget binds"
        )
    if lines >= MEMORY_INDEX_MAX_LINES * MEMORY_INDEX_WARN_RATIO:
        warnings.append(
            f"{path} is at {lines} of {MEMORY_INDEX_MAX_LINES} lines; merge or retire topics before the budget binds"
        )
    return {
        "index_bytes": size,
        "index_lines": lines,
        "budget_bytes": MEMORY_INDEX_MAX_BYTES,
        "budget_lines": MEMORY_INDEX_MAX_LINES,
        "headroom_bytes": MEMORY_INDEX_MAX_BYTES - size,
        "headroom_lines": MEMORY_INDEX_MAX_LINES - lines,
        "warnings": warnings,
    }


def retire_memory_topic(
    workspace_root: Path,
    slug: str,
    *,
    superseded_by: str = "",
    ...
    """Mark a topic retired (or active again) under `.memory.lock`, never deleting it.

    A retired topic leaves `MEMORY.md` and the default ranking but keeps its file, its incidents and
    its sources, so the lesson stays recoverable and the provenance stays true. Deletion is the one
    remedy this layer never offers.
    """
    if superseded_by and not MEMORY_SLUG_PATTERN.match(superseded_by):
        raise WorkspaceError(f"superseded_by must be a lowercase-hyphenated slug: {superseded_by!r}")
    if reactivate and superseded_by:
        raise WorkspaceError("choose --reactivate or --superseded-by")
    stamp = updated or datetime.now().astimezone().date().isoformat()
    if not MEMORY_DATE_PATTERN.match(stamp):
        raise WorkspaceError(f"updated {stamp!r} is not a YYYY-MM-DD date")
    try:
        with memory_lock(workspace_root, timeout=lock_timeout):
            ...
            if reactivate:
                ...
                topic.superseded_by = superseded_by or topic.superseded_by
            topic.updated = stamp
            atomic_write_text(path, render_memory_topic(topic, memory_topic_body(content)))
    except OSError as error:
        raise WorkspaceError(f"cannot retire {slug}: {error}") from error
    return {"path": str(path), "status": topic.status, "superseded_by": topic.superseded_by}
...
def memory_findings(workspace_root: Path, *, check_index: bool = False) -> ValidationReport:
    ...
    for topic in topics:
        if topic.superseded_by and topic.superseded_by not in names:
            report.warnings.append(
                f"{topic.path} says it is superseded by {topic.superseded_by!r}, which is not a topic in {directory}"
            )
