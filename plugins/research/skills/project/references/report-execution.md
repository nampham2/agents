...
Use `research-project show-graph <project-dir>` as the source. Include one row per task with ID/name,
dependency level, dependencies, status, effect, authorization, recorded evidence pass/fail counts,
and measured span or `unmeasured`. Explain consequential dependencies and execution changes.
