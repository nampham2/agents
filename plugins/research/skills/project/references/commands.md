...
`read <project-dir> spec --outline` returns headings and the whole-document SHA-256 token.
...
may replace any unique non-overlapping sections. Reflection uses `--body-file -`:
...
Supply `--entry-id` for retry-safe appends. The same ID and body returns the existing entry; different
content conflicts. Decisions and findings are not command evidence.
...
Keep that draft, reload context, and retry against the reported state; do not rewrite the reflection.
If `committed: true` accompanies an index or final-validation error, do not retry `close`; run the
reported `rebuild-index` and closure-validation recovery instead.
