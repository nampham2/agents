...
| S4-18 | **Accepted.** §9.2 defines `external` keys with explicit escaping of slash segments, rejects empty keys, and states that no case or URI normalization is applied — so two spellings of one external resource are two claims, which is stated as a limitation rather than papered over. |
...
| S4-28 | **Accepted.** §15.1 bounds result and rationale sizes and `log_cap` per stream per capture; §11.2 states that streams are drained concurrently to bounded buffers with redaction applied before the bytes are written, so neither memory nor a blocked pipe is unbounded. §21.3 adds journal I/O volume and evidence-read cost to what the benchmark must measure, alongside a tiny-task overhead budget and a linear-graph case. |
